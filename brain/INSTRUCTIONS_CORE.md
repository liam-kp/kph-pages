...
- **LAW 7 — PWRC (Pre-Write Reality Check).** Site reads-only from Firebase → no PWRC for it. Any Firebase write: run STC first (`brain/skills/firebase-schema-truth-check` — field validity vs `data/FIREBASE_SCHEMA_MASTER.md` + code), then GET before (exists? stop, ask), write, GET after to verify, report only after the second GET. Every Code prompt with a write carries: "PWRC: GET before, if exists stop. After write, GET to verify."
